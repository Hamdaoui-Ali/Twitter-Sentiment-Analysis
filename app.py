...
def load_model_and_vectorizer():
    with open('model_corrected.pkl', 'rb') as model_file:
        model = pickle.load(model_file)
    with open('vectorizer_corrected.pkl', 'rb') as vectorizer_file:
        vectorizer = pickle.load(vectorizer_file)
    return model, vectorizer

# Define sentiment prediction function
def predict_sentiment(text, model, vectorizer, stop_words):
    # Preprocess text (matching training preprocessing - NO stemming was used)
    text = re.sub('[^a-zA-Z]', ' ', text)  # Remove non-alphabetic characters
    text = text.lower()  # Convert to lowercase
    text = text.split()  # Split into words
    text = [word for word in text if word not in stop_words]  # Remove stopwords only (no stemming)
    text = ' '.join(text)  # Join words back into a single string
    text = [text]  # Convert to list
    text = vectorizer.transform(text)  # Transform using the trained vectorizer
    ...
    if option == "Input text":
        ...
        if st.button("Fetch Tweets"):
            try:
                ...
                if 'tweets' in tweets_data and tweets_data['tweets']:  # Check if the 'tweets' key exists and has data
                ...
                st.info("Note: Twitter scraping may not work due to API limitations. Try using the 'Input text' option instead.")
